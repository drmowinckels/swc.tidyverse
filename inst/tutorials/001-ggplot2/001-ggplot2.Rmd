...
```{r challenge_1a-hint-1}
The name of the data object is `penguins`
```

```{r challenge_1a-hint-2}
...
```{r challenge_1b, exercise = TRUE}
ggplot(data = penguins) +
  geom___(mapping = aes(x = year, 
                        y = __bill_length_mm))
```

```{r challenge_1b-solution}
ggplot(data = penguins) +
  geom_jitter(mapping = aes(x = year, 
                            y = __bill_length_mm))
```

```{r challenge_1b-check}
grade_code(
  correct = random_praise(),
  incorrect = random_encouragement()
)
...
```{r challenge_1c, exercise = TRUE}
ggplot(data = penguins) +
  geom_jitter(mapping = aes(x = __, 
                            y = __bill_length_mm))
```

```{r challenge_1c-solution}
ggplot(data = penguins) +
  geom_jitter(mapping = aes(x = island,
                            y = __bill_length_mm))
```

```{r challenge_1c-check}
grade_code(
  correct = random_praise(),
  incorrect = random_encouragement()
)
...
```{r challenge_3-hint-1}
Try adding island to shape
```

```{r challenge_3-hint-2}
...
```{r challenge_4d-hint-1}
move `colour = "blue"`, into the `aes()` 
```

```{r challenge_4d-hint-2}
ggplot(data = penguins) + 
  geom_point(
    mapping = aes(x = body_mass_g, 
                  y = bill_length_mm,
...
```{r challenge_6c-hint-1}
Try geom_density
```

```{r challenge_6c-hint-2}
...
```{r challenge_8a-hint-1}
try changing `year` to `factor(year)`
```

```{r challenge_8a-hint-2}
...
```{r challenge_8b-hint-1}
try changing `year` to `factor(year)`
```

```{r challenge_8b-hint-2}
When year is a factor, we now need a colour palette that is "discrete" and not "continuous". 
Try using `scale_fill_viridis_d()`. 
```
...
```{r challenge_9b, exercise = TRUE, exercise.lines = 20}
ggplot(penguins) +
  geom_boxplot(
    mapping = aes(y = body_mass_g, 
                  x = factor(year),
