...
>
> **1c**: Transform all the colmns with milimetres measurements so they are scaled, and the _prefix_ `sc_` to the columns names.
...
>
> **2d**: Calaculate the centered bill length again, but this time also include a new column `n` with the number of *total* observations (hint: you can use the `n()` function). Why does the inclusion of an `ungroup()` step a difference?
