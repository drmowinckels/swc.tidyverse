...
that looks ok, but the code is horrible! And what if you have even more conditionals? Thankfully, dplyr has a smarter way of doing this, called `case_when`. This function is similar to `if_else`, but where you specify what each condition should be assigned.
...
In our case, let us say we want to scale the three columns with millimetre measurements so that they have a mean of 0 and standard deviation of 1. We've alrady used the `scale` function once before, so we will do it again.
...
> **6a**: What is the difference in the minimum body mass for the penguins data sat if the data is grouped by species, and when it is ungrouped?
