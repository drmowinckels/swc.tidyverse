...
We've been grouping a lot and not ungrouping. Which might seem fine now, because we have not really done anything more after the summarise. But in many cases we might continue our merry data handling way and do lots more, and then the preserving of the grouping can give us some unexpected results. Let us explore that a little.
...
```
When we group by a single column and summarise, the output data is no longer grouped. In a way, the "summarise" uses up one group while summarising, as based on species, the data can not be condensed any further than this.
